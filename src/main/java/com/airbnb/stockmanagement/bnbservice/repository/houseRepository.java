...
public interface houseRepository extends JpaRepository<house, Long> {
    ...
    @Query("SELECT h FROM house h WHERE h.deleted = false AND h.houseMaxguest >= :houseMaxguest AND h.productCreatedDate >= :startDate AND h.productUpdateDate <= :endDate")
    List<house> findAvailableHouses(@Param("startDate") Date startDate, @Param("endDate") Date endDate, @Param("houseMaxguest") int houseMaxguest);
